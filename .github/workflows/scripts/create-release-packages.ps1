...
function Build-Variant {
    ...
    # Create zip archive
    $zipFile = Join-Path $GenReleasesDir "spec-kit-abap-template-${Agent}-${Script}-${Version}.zip"
...
Get-ChildItem -Path $GenReleasesDir -Filter "spec-kit-abap-template-*-${Version}.zip" | ForEach-Object {
    Write-Host "  $($_.Name)"
}
