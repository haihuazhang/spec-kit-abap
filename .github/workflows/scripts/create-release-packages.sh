...
build_variant() {
  ...
  ( cd "$base_dir" && zip -r "../spec-kit-abap-template-${agent}-${script}-${NEW_VERSION}.zip" . )
  echo "Created $GENRELEASES_DIR/spec-kit-abap-template-${agent}-${script}-${NEW_VERSION}.zip"
}
...
ls -1 "$GENRELEASES_DIR"/spec-kit-abap-template-*-"${NEW_VERSION}".zip
